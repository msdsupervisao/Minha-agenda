...
function reminderToRow(item: Reminder, userId: string) { return { id: item.id, user_id: userId, contact_id: item.contactId, title: item.title, due_at: item.dueAt, notification_status: item.notificationStatus, created_at: item.createdAt, updated_at: item.updatedAt, metadata: {} }; }
...
function contactFromRow(row: Record<string, unknown>): Contact { return { ...base(row), name: String(row.name), aliases: Array.isArray(row.aliases) ? row.aliases.map(String) : [], role: stringOrNull(row.role), className: stringOrNull(row.class_name), phone: stringOrNull(row.phone), whatsappOptIn: Boolean(row.whatsapp_opt_in), lastInboundAt: stringOrNull(row.last_inbound_at) }; }
function expenseFromRow(row: Record<string, unknown>): Expense { return { ...base(row), amount: Number(row.amount), currency: 'BRL', category: String(row.category), occurredAt: String(row.occurred_at), source: row.source === 'voice' ? 'voice' : 'text' }; }
function taskFromRow(row: Record<string, unknown>): Task { return { ...base(row), title: String(row.title), status: row.status === 'done' ? 'done' : 'open', dueAt: stringOrNull(row.due_at), contactId: stringOrNull(row.contact_id) }; }
function reminderFromRow(row: Record<string, unknown>): Reminder { return { ...base(row), title: String(row.title), dueAt: String(row.due_at), contactId: stringOrNull(row.contact_id), notificationStatus: row.notification_status === 'delivered' ? 'delivered' : 'pending' }; }
function eventFromRow(row: Record<string, unknown>): CalendarEvent { return { ...base(row), title: String(row.title), startsAt: String(row.starts_at), endsAt: stringOrNull(row.ends_at), contactId: stringOrNull(row.contact_id) }; }
function noteFromRow(row: Record<string, unknown>): Note { return { ...base(row), content: String(row.content), contactId: stringOrNull(row.contact_id) }; }
