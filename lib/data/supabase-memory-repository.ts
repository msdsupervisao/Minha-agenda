...
export function reminderFromRow(row: Record<string, unknown>): Reminder { return { ...base(row), title: String(row.title), dueAt: String(row.due_at), contactId: stringOrNull(row.contact_id), notificationStatus: row.notification_status === 'delivered' ? 'delivered' : 'pending' }; }
